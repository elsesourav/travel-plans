...
const tagsMap = {
  ...
  'Ziro Valley (Arunachal Pradesh)': ['Tribal', 'Nature', 'Music Festival', 'Offbeat'],
  ...
  'Tawang (Arunachal Pradesh)': ['Buddhist', 'Mountain', 'Adventure', 'Monastery'],
  'Spiti Valley (Himachal Pradesh)': ['Adventure', 'Buddhist', 'Mountain', 'Stargazing'],
...
function extractState(dest) {
  if (dest.state) return dest.state.trim();
...
function extractAttractions(groupOf6) {
  ...
  for (const item of groupOf6.breakdown) {
    ...
    if (foundKeyAttractions && item.category && item.category.startsWith('-')) {
      ...
      const cleaned = item.category.replace(/^-\s*\*?\*?/, '').replace(/\*?\*?$/, '').trim();
...
function parseBudget(total) {
  ...
  if (typeof total === 'object' && total.low && total.typical) {
    ...
    const typical = parseInt(String(total.typical).replace(/[^0-9]/g, '')) || 20000;
...
function convertDestination(dest) {
  ...
  const images = imageMap[dest.destination] || imageMap[name] || [
    'https://images.unsplash.com/photo-1501785888041-af3ef285b470?w=1920&q=80',
    'https://images.unsplash.com/photo-1470071459604-3b5ec3a7fe05?w=1200&q=80',
    'https://images.unsplash.com/photo-1441974231531-c6227db76b6e?w=1200&q=80',
    'https://images.unsplash.com/photo-1469474968028-56623f02e42e?w=1200&q=80',
  ...
  const tagline = taglineMap[dest.destination] || taglineMap[name] || `Discover ${name}`;
  const tags = tagsMap[dest.destination] || tagsMap[name] || ['Nature', 'Adventure', 'Offbeat'];
  
  const landscape = (dest.landscape || '').trim();
  const permits = (dest.permits || 'Not required for Indian citizens').trim();
  const permitRequired = permits.toLowerCase().includes('required') && !permits.toLowerCase().includes('not required');
  ...
  const groupCosts = {
    costs: (dest.groupOf6?.breakdown || [])
      .filter(b => b.perPerson && !b.category.includes('Key Attractions') && !b.category.startsWith('-'))
      .map(b => {
  ...
  return {
    ...
    coordinates: `${dest.coordinates?.latitude || ''}${dest.coordinates?.longitude ? ', ' + dest.coordinates.longitude : ''}`.replace(/\s+/g, ' ').trim(),
    landscape,
    permits,
    permitRequired,
    duration: '7 Days',
...
export const destinations: Destination[] = [\n`;
  ...
  for (const dest of converted) {
    output += `  ${JSON.stringify(dest, null, 2).split('\n').map((line, i) => i === 0 ? line : '  ' + line).join('\n')},\n`;
  }
...
console.log(`✅ Generated destinations.ts with ${allDestinations.length} destinations`);
